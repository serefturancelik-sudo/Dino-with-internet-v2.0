...
pygame.init()
pygame.font.init()
pygame.mixer.init() # SİSTEM: Ses mikserini başlatıyoruz
...
pygame.display.set_caption("Dinozor: Ay Operasyonu")
...
COLOR_MOON_BG = (40, 44, 52)      
COLOR_MOON_GROUND = (120, 124, 133) 
COLOR_TEXT = (83, 83, 83)        
...
COLOR_BLACK = (10, 10, 12)        

# --- SES EFEKTLERİ YÜKLEME ---
# Eğer dosyalarınız yoksa oyunun çökmemesi için try-except bloğuna almak en güvenlisidir.
try:
    snd_jump = pygame.mixer.Sound("zip_ses.wav")
    snd_laser = pygame.mixer.Sound("lazer_ses.wav")
    snd_explosion = pygame.mixer.Sound("patlama_ses.wav")
    snd_gameover = pygame.mixer.Sound("game_over_ses.wav")
    
    # Ses seviyelerini biraz dengeleyelim (0.0 ile 1.0 arası)
    snd_jump.set_volume(0.4)
    snd_laser.set_volume(0.3)
    snd_explosion.set_volume(0.5)
    snd_gameover.set_volume(0.6)
    
    # Arka plan müziğini yükle ve döngüye al (-1 sonsuz döngü demektir)
    pygame.mixer.music.load("arka_plan.mp3")
    pygame.mixer.music.set_volume(0.2) # Müziğin sesi efektleri bastırmasın
    pygame.mixer.music.play(-1)
except pygame.error as e:
    print(f"Ses dosyaları yüklenemedi (Kod yine de çalışacak): {e}")
    # Dosyalar yoksa hata vermemesi için boş taklit objeler atıyoruz
    class DummySound:
        def play(self): pass
    snd_jump = snd_laser = snd_explosion = snd_gameover = DummySound()
...
def draw_creepy_crawler(surf, x, y, frame=0):
    wave = int(math.sin(frame * 0.3) * 6)
    pygame.draw.ellipse(surf, COLOR_BLACK, (x, y + 20 + wave, 25, 15))
    pygame.draw.ellipse(surf, COLOR_BLACK, (x + 18, y + 22 - wave, 20, 13))
    pygame.draw.ellipse(surf, COLOR_BLACK, (x + 32, y + 24 + wave, 15, 10))
    pygame.draw.circle(surf, COLOR_RED, (x + 5, y + 24 + wave), 3)
    pygame.draw.line(surf, COLOR_BLACK, (x + 10, y + 30 + wave), (x + 5, y + 38 + wave), 3)
    pygame.draw.line(surf, COLOR_BLACK, (x + 22, y + 32 - wave), (x + 25, y + 40 - wave), 3)
...
class Game:
    ...
    def trigger_game_over(self):
        """Oyun bittiğinde çağrılan ve müzik/ses yönetimini tek yerden yapan fonksiyon"""
        self.state = STATE_GAME_OVER
        pygame.mixer.music.stop() # Müziği durdur
        snd_gameover.play()       # Kaybetme sesini çal
    ...
    def reset_game_states(self):
        ...
        # Eğer müzik durmuşsa yeniden başlat
        ...
        self.crawlers = []
        self.crawlers_defeated = 0
        self.needed_crawler_kills = 15
        self.mars_allies_y = 440
    ...
    def update_timer(self):
        if self.state in [STATE_CLASSIC, STATE_WIFI_BOMB, STATE_PUZZLE, STATE_HELICOPTER, STATE_MARS_TRAVEL, STATE_MARS_BATTLE, STATE_MOON_SURFACE] and not self.is_paused:
    ...
    def draw_hud(self):
        ...
        if self.state == STATE_CLASSIC:
            ...
            status_text_str = f"Marslıları Temizle: {self.martians_defeated}/{self.needed_martian_kills}"
        elif self.state == STATE_MOON_SURFACE:
            status_text_str = f"AY İŞGALİ! Siyah Sürüngenleri Vur: {self.crawlers_defeated}/{self.needed_crawler_kills}"
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.state in [STATE_CLASSIC, STATE_WIFI_BOMB, STATE_PUZZLE, STATE_HELICOPTER, STATE_MARS_TRAVEL, STATE_MARS_BATTLE, STATE_MOON_SURFACE]:
                    if not self.is_paused and self.pause_icon_rect.collidepoint(mouse_pos):
                        self.is_paused = True
                        pygame.mixer.music.pause() # Oyunu durdurunca müziği de duraklat
                        self.pause_start_ticks = pygame.time.get_ticks()
                        continue
                    elif self.is_paused and self.resume_btn_rect.collidepoint(mouse_pos):
                        self.is_paused = False
                        pygame.mixer.music.unpause() # Oyuna dönünce müziği devam ettir
                ...
                if self.state == STATE_MENU:
                ...
                elif self.state in [STATE_VICTORY, STATE_GAME_OVER]:
                    self.state = STATE_MENU
                    
                elif self.state == STATE_MARS_KING:
                    self.state = STATE_MOON_SURFACE
                    self.dino_x = 150
                    self.dino_y = 440
                    ...
                    if not self.is_jumping:
                        ...
                        snd_jump.play() # SES: Klasik zıplama sesi tetiklendi
                ...
                elif self.state in [STATE_MARS_BATTLE, STATE_MOON_SURFACE]:
                    # SES: Lazer mermisi ateşleme sesi tetiklendi
                    snd_laser.play()
    ...
    def update_classic_mode(self):
        ...
        for cactus in self.cacti[:]:
            ...
            if dino_rect.colliderect(cactus):
                self.trigger_game_over() # SES: Müzik durdurulup kaybetme sesi tetiklenir
    ...
    def update_moon_surface_mode(self):
        ...
        if mouse_pressed[0]:
            ...
            if mouse_x < self.dino_x:
                ...
                self.dino_x += 6
        self.dino_x = max(120, min(self.dino_x, SCREEN_WIDTH // 2)) 
        ...
        if random.randint(1, 35) == 1 and len(self.crawlers) < 5:
            cx = SCREEN_WIDTH + 50
            cy = 445 
            self.crawlers.append(pygame.Rect(cx, cy, 45, 35))
        ...
        for crawler in self.crawlers[:]:
            ...
            dino_rect = pygame.Rect(self.dino_x, self.dino_y, self.dino_width, self.dino_height)
            if dino_rect.colliderect(crawler):
                self.trigger_game_over()
                
            cr_rect = pygame.Rect(crawler.x, crawler.y, 45, 35)
            for proj in self.projectiles[:]:
                p_rect = pygame.Rect(proj["x"], proj["y"], 10, 5)
                if p_rect.colliderect(cr_rect):
                    if proj in self.projectiles: self.projectiles.remove(proj)
                    if crawler in self.crawlers: self.crawlers.remove(crawler)
                    snd_explosion.play() # SES: Sürüngen vurulduğunda patlama sesi
                    self.crawlers_defeated += 1
                    
        if self.crawlers_defeated >= self.needed_crawler_kills:
            self.state = STATE_VICTORY
            pygame.mixer.music.stop() # Zafer anında arka plan müziğini kapatabilirsin
    ...
    def draw(self):
        ...
        elif self.state == STATE_MOON_SURFACE: self.draw_moon_surface_stage()
        elif self.state == STATE_VICTORY: self.draw_victory_stage()
        elif self.state == STATE_GAME_OVER: self.draw_game_over()
        
        if self.state in [STATE_CLASSIC, STATE_WIFI_BOMB, STATE_PUZZLE, STATE_HELICOPTER, STATE_MARS_TRAVEL, STATE_MARS_BATTLE, STATE_MOON_SURFACE]:
            if not self.is_paused:
                line_color = COLOR_WHITE if self.state in [STATE_PUZZLE, STATE_MARS_TRAVEL, STATE_MARS_BATTLE, STATE_MOON_SURFACE] else COLOR_TEXT
    ...
    def draw_menu(self):
        ...
        info_text_1 = self.font_medium.render("DÜNYADAN KAÇIŞ VE AY OPERASYONU!", True, COLOR_RED)
        info_text_2 = self.font_small.render("Kaktüsler, bombalar, helikopterler ve Ay'daki simsiyah sürüngenler.", True, COLOR_TEXT)
    ...
    def draw_mars_king_stage(self):
        ...
        alert_text = self.font_medium.render("DİKKAT! Başka evrenden gelen uzaylılar Mars'ı işgal etti!", True, COLOR_RED)
        escape_text = self.font_small.render("[ 2 MARSLI DOSTUNLA AY'A KAÇMAK İÇİN EKRANA DOKUNUN! ]", True, COLOR_WHITE)
        
        screen.blit(win_title, (SCREEN_WIDTH // 2 - win_title.get_width() // 2, 80))
        screen.blit(alert_text, (SCREEN_WIDTH // 2 - alert_text.get_width() // 2, 160))
        screen.blit(escape_text, (SCREEN_WIDTH // 2 - escape_text.get_width() // 2, 240))

    def draw_moon_surface_stage(self):
        screen.fill(COLOR_MOON_BG)
        pygame.draw.rect(screen, COLOR_MOON_GROUND, (0, 490, SCREEN_WIDTH, SCREEN_HEIGHT - 490))
        pygame.draw.line(screen, COLOR_WHITE, (0, 490), (SCREEN_WIDTH, 490), 3)
        
        draw_martian(screen, self.dino_x - 60, self.mars_allies_y, self.heli_frame)
        draw_martian(screen, self.dino_x - 110, self.mars_allies_y, self.heli_frame + 10)
        
        draw_dino(screen, self.dino_x, self.dino_y, self.dino_frame, False)
        pygame.draw.rect(screen, COLOR_RED, (self.dino_x + self.dino_width - 5, self.dino_y + 15, 15, 6)) 
        
        for proj in self.projectiles:
            pygame.draw.line(screen, COLOR_GREEN, (proj["x"], proj["y"]), (proj["x"] + 15, proj["y"]), 4)
            
        for crawler in self.crawlers:
            draw_creepy_crawler(screen, crawler.x, crawler.y, self.heli_frame)
            
        self.draw_hud()

    def draw_victory_stage(self):
        screen.fill(COLOR_DARK_BG)
        vic_title = self.font_large.render("NİHAİ ZAFER! AY TEMİZLENDİ", True, COLOR_GREEN)
        score_text = self.font_medium.render("Sadık iki Marslınla birlikte Ay'ı simsiyah yaratıklardan kurtardın!", True, COLOR_WHITE)
        restart_text = self.font_small.render("[ EN BAŞTAN BAŞLAMAK İÇİN EKRANA DOKUNUN ]", True, COLOR_BLUE)
        
        screen.blit(vic_title, (SCREEN_WIDTH // 2 - vic_title.get_width() // 2, 150))
    ...
    def draw_game_over(self):
        ...
        reason_text = self.font_medium.render("Yaratıklar veya engeller seni alt etti!", True, COLOR_TEXT)
